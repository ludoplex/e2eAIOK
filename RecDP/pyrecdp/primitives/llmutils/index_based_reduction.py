...
def index_based_reduction_spk(src_df, dup_df, enable_hash):
    return (
        src_df.join(dup_df, ["doc_id", "hash"], "anti")
        if enable_hash
        else src_df.join(dup_df, "doc_id", "anti")
    )
...
def index_based_reduction(in_dir, dup, out_dir, enable_hash = True, spark = None):
    ...
    if large_files := [f for f in list_files if os.path.getsize(f) > 2**30]:
        ...
        src_df = src_df_large if first else src_df.union(src_df_large)
    else:
...
if __name__ == "__main__":
    ...
    with Timer("apply duplicates.pickle to create new data"):
