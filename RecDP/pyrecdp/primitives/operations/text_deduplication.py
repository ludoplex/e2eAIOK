...
def normalize_str(s):
    s = ftfy.fix_text(s, normalization="NFC") if s else ""
    return s
...
class FuzzyDeduplicateApplyDict(BaseLLMOperation):
    ...
    def process_spark(self, spark, spark_df: DataFrame, global_df: DataFrame) -> DataFrame:
        ...
        spark_df = (
            spark_df.select(
                'norm_text', *[n for n in column_names if n != 'norm_text']
            )
            if 'norm_text' in column_names
            else spark_df.select('text', *[n for n in column_names if n != 'text'])
        )
        return spark_df.join(global_df, 'global_id', 'left_anti')
...
def get_hash_indexing_spk(spark_df, global_id_name = 'global_id'):
    return spark_df.groupby('hash').agg(
        F.collect_list(global_id_name).alias(f'{global_id_name}_list'),
        F.count("hash").alias('hash_count'),
    )
...
def index_based_reduction_spk(src_df, dup_df, enable_hash, global_id_name = 'global_id'):
    return (
        src_df.join(dup_df, [global_id_name, "hash"], "left_anti")
        if enable_hash
        else src_df.join(dup_df, global_id_name, "left_anti")
    )
